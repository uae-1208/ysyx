...
void RET_Log(uint32_t last_PC, uint32_t current_pc)
{
  // Log("s->pc:%x, s->dnpc:%x", last_PC, current_pc);
  loop--;
  //get the FUNC symbol index in sym_fun_group
  uint32_t index = check_func_interval(current_pc);   
  fprintf(ftrace_log, "[ftrace] 0x%08x: ", last_PC); 
...
void J_Log(uint32_t last_PC, uint32_t current_pc)
{
  // Log("s->pc:%x, s->dnpc:%x", last_PC, current_pc);
  //get the FUNC symbol index in sym_fun_group
  uint32_t index = check_func_interval(current_pc);
